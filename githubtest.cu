...
size_t loadImageFile(unsigned char *grey_image, const std::string &input_file, int *rows, int *cols) 
{
	...
	img_data = cv::imread(input_file.c_str(), IMREAD_COLOR);
	if (img_data.empty()) 
	{
	...
	*cols = img_data.cols;
    ...
    outputImage("T.jpg", h_rgb_image, img_data.rows, img_data.cols);
...
void outputImage(const std::string& output_file, unsigned char* grey_image, int rows, int cols)
{
	...
	cv::Mat greyData(rows, cols, CV_8UC3,(void *) grey_image);
